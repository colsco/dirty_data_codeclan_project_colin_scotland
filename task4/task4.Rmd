...
# Canada cleanup ----
regex_1 <- regex_1 %>% 
  mutate(fixed = str_replace_all(fixed, pattern = "(?i)cana*d*a*e*[:symbol:]*|(?i)cascadia|(?i)Soviet Canuckistan", "Canada"))


# USA cleanup ----

regex_1 <- regex_1 %>% 
  mutate(fixed = str_replace_all(fixed, pattern = "(?i)T*h*e*[A-Za-z]*u+n+i+t+e+d+\\s+s+t+[ae]+t+e+s+\\s*of\\s*[Aa]merica[A-Za-z]*|(?i)[a-z]*USA[:punct:]*|[Nn][:punct:]\\s*America|(?i)usa+|(?i)america|^[A-Z][a-z]{3}[:punct:]{4}[A-Za-z]{6}|(?i)[:punct:]*[:symbol:]*m+[eu]+r+[ricka]*|(?i)unite*s*d*\\s*s*t*a*s*t*e*s+|New\\s+[JY][:alpha:]+$|Sub-Canadaian\\sNorth\\sUSA[:punct:]+\\s+USA|[Tt]he\\s+[Bb]est\\s+[Oo]ne\\s*[:punct:]\\s*USA|[Uu]+[\\s\\.]+[Ss]+[\\s\\.]*\\s*[Aa]*\\s*\\.*|t[ae]+te+[ads]*$|USAUSA$|Alaska|California|North Carolina|Pittsburgh", "USA"))

regex_1 <- regex_1 %>% # break it up a bit for sanity's sake
  mutate(fixed = str_replace_all(fixed, pattern = "^[:alpha:]+\\s+S+U+S+A+|^U+S+A+U+S+A+|U+s+|U+S+A+\\s+[Oo]+f+\\s+A+|U+S+A+a+|U+S+A+\\s*[:alpha:]+","USA"))
