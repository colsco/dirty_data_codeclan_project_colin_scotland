...
regex_1 <- regex_1 %>% 
  mutate(fixed = str_replace_all(fixed, pattern = "(?i)cana*d*a*e*|(?i)cascadia", "Canada"))

# Other general cleans

#regex_1 <- regex_1 %>% 
#  mutate(fixed = str_replace_all(fixed, pattern = "[0-9]", is.na()))
...
#regex_1 <- regex_1 %>% 
#  mutate(str_replace_all(country, pattern = "(?i)[A-Za-z ]*u+n+i+t+e+d+ +s+t+a+t+e+s+[A-Za-z]*|(?i)[a-z]*USA[:punct:]*|N. #America|(?i)usa+|(?i)america|^[A-Z][a-z]{3}[:punct:]{4}[A-Za-z]{6}|(?i)m+[eu]+r+[ricka]*|(?i)i pretend [a-z0-9 [:punct:]]*"
#                     , "USA"))
